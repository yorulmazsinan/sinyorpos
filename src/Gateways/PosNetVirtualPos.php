...
/**
 * Class PosNetVirtualPos
 */
class PosNetVirtualPos extends AbstractGateway
{
	protected const HASH_ALGORITHM = 'sha256';
	protected const HASH_SEPARATOR = ';';
	public const NAME = 'PosNetVirtualPos';
	/**
	 * Response Codes
	 * @var array
	 */
	...
	/** @var PosNetVirtualPosAccount */
	protected $account;
	/** @var AbstractCreditCard|null */
	protected $card;
	/** @var Request */
	protected $request;
	/** @var PosNetCryptVirtualPos|null */
	private $crypt;
	/** @var PosNetVirtualPosRequestDataMapper */
	protected $requestDataMapper;

	/**
	 * @param PosNetVirtualPosAccount $account
	 * @param PosNetVirtualPosRequestDataMapper $requestDataMapper
	 */
	...
	{
		$this->crypt = new PosNetCryptVirtualPos();
	...
	public function make3DPayment(Request $request)
	{
		...
		if ($this->check3DHash($request->all())) {
			...
			$requestData = $this->requestDataMapper->create3DResolveMerchantRequestData($this->account, $this->order, $request->all());
	 ...
	 * @return bool
	 */
	public function verifyResponseMAC(PosNetVirtualPosAccount $account, $order, $data): bool
	{
		$hashStr = '';
	...
	{
		if (!($this->crypt instanceof PosNetCryptVirtualPos)) {
			return false;
		}
			...
			if (is_array($rawResponseData->transactions->transaction) && count($rawResponseData->transactions->transaction)) {
				...
				if (count($rawResponseData->transactions->transaction) > 1) {
					...
					foreach ($rawResponseData->transactions->transaction as $key => $_transaction) {
						if ($key > 0) {
							$currency = isset($currencies[$_transaction->currencyCode]) ? (string)$currencies[$_transaction->currencyCode] : $_transaction->currencyCode;
							$refunds[] = [
