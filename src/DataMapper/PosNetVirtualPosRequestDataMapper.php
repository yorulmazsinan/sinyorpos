...
/**
 * Creates request data for PosNetVirtualPos Gateway requests
 */
class PosNetVirtualPosRequestDataMapper extends AbstractRequestDataMapper
{
	public const CREDIT_CARD_EXP_DATE_FORMAT = 'ym';
	protected const HASH_ALGORITHM = 'sha256';
	protected const HASH_SEPARATOR = ';';
	/**
	 * PosNetVirtualPos requires order id with specific length
	 */
	...
	public function create3DFormData(AbstractPosAccount $account, $order, string $txType, string $gatewayURL, ?AbstractCreditCard $card = null, $extraData = null): array
	{
		$inputs = [
			...
			//todo bunun account icine veya order icin tasinmasi gerekiyor
			'posnetData'        => $extraData['data1'],
			//Ödeme bilgilerini içermektedir.
			'posnetData2'       => $extraData['data2'],
			//Kart bilgileri request içerisinde bulunuyorsa bu alan oluşturulmaktadır
			'digest'            => $extraData['sign'],
			//Servis imzası.
			'merchantReturnURL' => $order->success_url,
			'url'               => '',
			//todo belki kaldirabiliriz
	 ...
	 * @return string
	 */
	public function createSecurityData(PosNetVirtualPosAccount $account): string
	{
		$hashData = [
			$account->getStoreKey(),
